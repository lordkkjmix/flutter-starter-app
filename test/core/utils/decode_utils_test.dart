...
void main() {
  ...
  group('decodeDataSourceResponse', () {
    ...
    test('should throw an exception if decoding fails', () {
      final response = 'invalid_response'; // Invalid response type

      expect(
          () => decodeDataSourceResponse(response, decoder: (item) {
  ...
  group('implementRepository', () {
    ...
    test('should return Left ServerFailure when implementation fails',
        () async {
      final implementation = Future.error(const RequestException(
        statusCode: 500,
      ...
      expect(result, isA<Left<Failure, DeviceModel>>());
      expect(
